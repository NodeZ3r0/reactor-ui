...
# Run it (connects to existing  network)
docker run -d \
  --name reactor_ui \
...
docker run -d --name reactor_ui --network wopr_ollama_chat__net -p 5173:80 reactor-ui:latest
```
...
# Check network
docker network inspect wopr_ollama_chat__net | grep -A5 reactor_ui
```
...
- [ ] API connectivity working: `docker exec reactor_ui curl http:///health`
